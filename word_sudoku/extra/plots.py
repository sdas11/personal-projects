...
def __render_scatter_plot(df_ga):
    ...
    facet_grid = sns.FacetGrid(df_ga, col="sel_strat", row="crsvr") \
        .map(plt.scatter, "max_population", "success_rate", alpha=0.5).add_legend()
    return facet_grid
...
if __name__ == '__main__':
    ...
    df_easy = df_easy[df_easy['mutation_probability'] == 0.3]
    df_hard = df[df['problem_difficulty'] == 'hard']
    df_hard = df_hard[df_hard['mutation_probability'] == 0.3]
